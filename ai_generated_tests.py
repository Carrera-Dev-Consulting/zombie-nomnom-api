...
class TestGraphQLIntegration:
    ...
    def test_query_all_games_with_existing_games(self, client: TestClient, clean_games):
        ...
        # Create first game
        response1 = self.graphql_request(client, create_query, {"players": ["Alice", "Bob"]})
        game1_id = response1.json()["data"]["createGame"]["game"]["id"]
        
        # Create second game
        response2 = self.graphql_request(client, create_query, {"players": ["Charlie", "Dave"]})
    ...
    def test_query_specific_game_by_id(self, client: TestClient, clean_games):
        ...
        create_response = self.graphql_request(client, create_query, {"players": ["Alice"]})
    ...
    def test_end_round_success(self, client: TestClient, clean_games):
        ...
        create_response = self.graphql_request(client, create_query, {"players": ["Alice", "Bob"]})
        game_id = create_response.json()["data"]["createGame"]["game"]["id"]
    ...
    def test_complete_game_workflow(self, client: TestClient, clean_games):
        ...
        alice_final_score = alice_end.json()["data"]["endRound"]["round"]["player"]["score"]
    ...
    def test_multiple_dice_draws_in_round(self, client: TestClient, clean_games):
        ...
        first_bag_size = len(first_draw.json()["data"]["drawDice"]["round"]["bag"]["dice"])
        ...
        if shotguns < 3:
            ...
            second_hand = second_draw.json()["data"]["drawDice"]["round"]["player"]["hand"]
            second_bag_size = len(second_draw.json()["data"]["drawDice"]["round"]["bag"]["dice"])
    ...
    def test_game_state_consistency(self, client: TestClient, clean_games):
        ...
        for initial_player in initial_players:
            matching_player = next(
                p for p in queried_game["players"] 
                if p["id"] == initial_player["id"]
            )
...
def run_full_game_test():
    ...
    print("\n=== Drawing Dice ===")
    draw_response = client.post("/", json={
        "query": """
        mutation DrawDice($gameId: ID!) {
            drawDice(gameId: $gameId) {
                errors
    ...
    print("\n=== Final Game State ===")
    final_response = client.post("/", json={
        "query": """
        query GetGame($gameId: ID!) {
            games(id: $gameId) {
                players {
